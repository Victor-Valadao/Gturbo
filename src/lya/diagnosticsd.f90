...
subroutine enediff(z,w,temp,ifr)
use paran
use commk
use lyap
use commphy
...
real(8) :: k2,ka,fac,ee,zz,scra,k,dk
real(8) :: dise_nu,dise_mu,disz_nu,disz_mu
integer :: i,j,ifr,ib
character(len=64) :: nome
...
ee=0.0d0
dk=dkx
!$acc enter data copyin(zz,ee)

!$acc parallel loop collapse(2) present(z,w,zz,ee,fkxs,fkys)
do 10 j=1,NY
do 10 i=1,NX2P1
 fac=1.d0
 if ((i.eq.1).or.(i.eq.NX2P1)) fac=0.5d0
...
!$acc update host(zz,ee)

temp=zz/(0.5d0*delta0**2)

! write(6,*)"Nt = ",ifr,", DeD = ",real(temp)
! call flush(6)

write(15,99)real(ifr*dt),real(temp),real(ee/(0.5d0*delta0**2))
call flush(15)
99 format(3g)

!$acc exit data delete(zz,ee) finalize
return
end

! ================================================================

subroutine enespec(z,w,ifr)
use paran
use commk
use commphy
use cudafor
...
!$acc update host(sp,sz,wn)
do ib=1,NBIN
 write(14,99)real(dk*(ib-1)),real(sp(ib)/wn(ib)),real(sz(ib)/wn(ib))
 99 format(3g)
end do
!$acc exit data delete(sp,sz,wn) finalize
return
